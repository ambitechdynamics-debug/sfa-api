...
export const memoryController = {
  create: async (req: Request, res: Response) => {
    const memory = await memoryService.create(req.user!.id, req.params.projectId, req.body);
    return sendSuccess(res, 'Memory created successfully', memory, 201);
  },

  list: async (req: Request, res: Response) => {
    const memories = await memoryService.list(req.user!.id, req.params.projectId);
