// icons.jsx — small inline SVG icon set, lucide-style stroke 1.5
// Usage: <Icon name="sparkles" size={18} />

const ICONS = {
  // brand mark — paper plane / flyer fold
  flyer: (
    <>
      <path d="M3 12L21 4l-4 18-4-8-7-2z" />
      <path d="M13 14l4-7" />
    </>
  ),
  // nav / generic
  ...
  upload:      <><path d="M12 20V8M6 12l6-6 6 6" /><path d="M4 20h16" transform="translate(0 -16)" /></>,
  ...
  google:      <><path d="M21.8 12.2c0-.7-.1-1.4-.2-2H12v3.9h5.5a4.7 4.7 0 0 1-2 3.1v2.6h3.3a10 10 0 0 0 3-7.6z" fill="currentColor" stroke="none"/><path d="M12 22a10 10 0 0 0 6.8-2.5l-3.3-2.6a6 6 0 0 1-9-3.1H3.1v2.6A10 10 0 0 0 12 22z" fill="currentColor" stroke="none" opacity=".75"/><path d="M6.5 13.8a6 6 0 0 1 0-3.6V7.6H3.1a10 10 0 0 0 0 8.8z" fill="currentColor" stroke="none" opacity=".5"/><path d="M12 6.4a5.4 5.4 0 0 1 3.8 1.5l2.9-2.9A10 10 0 0 0 3.1 7.6l3.4 2.6A6 6 0 0 1 12 6.4z" fill="currentColor" stroke="none" opacity=".85"/></>,
...
function Icon({ name, size = 18, stroke = 1.5, style, className, ...rest }) {
  ...
  return (
    <svg
      ...
      style={style} className={className} aria-hidden="true" {...rest}
    >
      {node}
    </svg>
  );
...
// Brand mark — small wordmark + paper-plane glyph
function BrandMark({ size = 20, withWordmark = true, color }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 9, color: color || "var(--ink-0)" }}>
      <span style={{
        width: size + 6, height: size + 6, borderRadius: 8,
        display: "inline-flex", alignItems: "center", justifyContent: "center",
        background: "linear-gradient(135deg, var(--acc-bright), var(--acc-deep))",
        color: "var(--acc-ink)",
        boxShadow: "inset 0 1px 0 rgba(255,255,255,0.25), 0 4px 12px rgba(224,138,100,0.3)",
      }}>
      ...
      {withWordmark && (
        <span style={{
          ...
          fontSize: size * 0.78, letterSpacing: "-0.02em",
          display: "inline-flex", alignItems: "baseline", gap: 4,
        }}>
          <span>Studio</span>
          <span style={{ color: "var(--acc)" }}>Flyer</span>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: size * 0.5, color: "var(--ink-2)", letterSpacing: "0.04em" }}>·AI</span>
        </span>
      )}
    </span>
  );
