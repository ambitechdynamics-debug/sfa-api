// sections-bot.jsx — Templates gallery, Use cases, How, Pricing, Reviews, Contact, Footer

/* ============================================================
   TEMPLATES — Win11 File-Explorer-like browser
   ============================================================ */
const TEMPLATE_PALETTES = {
  flyers: [
  ["#0a1418", "#16252d", "#4cc2ff", "#ffffff"],
  ["#1a1a1a", "#2d2d2d", "#76cbff", "#ffffff"],
  ["#0e1d23", "#1a3340", "#4cc2ff", "#e8f4fa"],
  ["#1f1f1f", "#3a3a3a", "#4cc2ff", "#ffffff"]],

  posters: [
  ["#1a1a1a", "#272727", "#4cc2ff", "#ffffff"],
  ["#0a0a0a", "#1a1a1a", "#76cbff", "#ffffff"]],

  cards: [
  ["#1a1a1a", "#2b2b2b", "#4cc2ff", "#ffffff"],
  ["#0a1418", "#16252d", "#76cbff", "#ffffff"]],
  ...
  cv: [
  ["#1a1a1a", "#ffffff", "#4cc2ff", "#1a1a1a"],
  ["#0a1418", "#f0f4f7", "#4cc2ff", "#0a1418"]],
...
function Templates({ t, lang }) {
  const [cat, setCat] = React.useState("all");
  const [format, setFormat] = React.useState(t.templates.formats[0]);
        ...
        <WindowChrome title={t.templates.windowTitle} icon={<Ico.templates style={{ color: "var(--accent)" }} />}>
          <div className="tpl-grid">
            ...
            <aside className="tpl-sb">
              <div style={{ padding: "12px 8px 4px", fontSize: 11, fontWeight: 600, color: "var(--tx-tertiary)", letterSpacing: ".06em", textTransform: "uppercase" }}>
                {lang === "fr" ? "Catégories" : "Categories"}
              </div>
              {t.templates.sidebar.map((it) => {
                const IconEl = Ico[it.icon] || Ico.grid;
                return (
                  <div key={it.id}
                  onClick={() => setCat(it.id)}
                  className={`sb-item ${cat === it.id ? "active" : ""}`}>
                    <IconEl />
            ...
            <div className="tpl-main">
              ...
              <div className="tpl-toolbar">
                <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--tx-secondary)", fontSize: 14 }}>
                  ...
                  <span style={{ fontFamily: "\"Segoe UI Variable\"" }}>{filtered.length} {t.templates.itemsLabel}</span>
                </div>
                <div style={{ flex: 1 }} />
                ...
                {filtered.map((it) => {
                  ...
                  return (
                    <div key={it.id} className="tpl-card">
                      <div style={{
                        background: pal[0],
                        aspectRatio: ["card"].includes(it.kind) ? "13/8" :
...
/* ============================================================
   USE CASES — B2B vs B2C as two side-by-side windows
   ============================================================ */
    ...
    <section id="uses" className="section" style={{ background: "linear-gradient(180deg, transparent, rgba(76,194,255,.02), transparent)" }}>
      <div className="container">
        ...
        <div className="uses-grid">
          <UseCaseCard data={t.uses.biz} icon={<Ico.building style={{ color: "var(--accent)" }} />} kind="biz" />
          <UseCaseCard data={t.uses.perso} icon={<Ico.user style={{ color: "var(--accent)" }} />} kind="perso" />
        </div>
      </div>
    </section>);
...
function UseCaseCard({ data, icon, kind }) {
  return (
    <WindowChrome
      title={data.chip}
      icon={icon}
      className="usecase-win">
      
      <div style={{ padding: "32px 32px 28px" }}>
        <h3 style={{ fontSize: 24, fontWeight: 600, lineHeight: 1.15, letterSpacing: -.015, marginBottom: 14 }}>
          {data.title}
        </h3>
      ...
      {/* Decorative footer band — mimics device info row */}
      <div style={{
        ...
        <>
            ...
            <div style={{ fontSize: 12, color: "var(--tx-tertiary)", marginLeft: "auto" }}>
              {window.__lang === "fr" ? "Équipe Boulangerie Pétrin" : "Boulangerie Pétrin team"}
            </div>
          </> :
        ...
        <>
            ...
            <div style={{ fontSize: 12, color: "var(--tx-secondary)" }}>
              {window.__lang === "fr" ? "Premier visuel en 4 min en moyenne" : "First visual in 4 min on average"}
            </div>
...
/* ============================================================
   PRICING — three side-by-side windows
   ============================================================ */
  ...
  return (
    <section id="pricing" className="section">
      <div className="container">
        ...
        {/* Monthly/Yearly toggle Win11-style */}
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 40 }}>
          <div style={{
            ...
            {[["monthly", t.pricing.monthly], ["yearly", t.pricing.yearly]].map(([k, lbl], i) => {
              const isActive = k === "yearly" && annual || k === "monthly" && !annual;
              return (
                <button key={k}
                onClick={() => setAnnual(k === "yearly")}
                style={{
...
function PricingCard({ plan, annual, lang }) {
  ...
  return (
    <WindowChrome
      title={plan.name}
      icon={plan.highlight ? <Ico.sparkle style={{ color: "var(--accent)" }} /> : <Ico.templates style={{ color: "var(--tx-tertiary)" }} />}
      ...
      <div style={{ padding: "28px 28px 28px", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 22 }}>
          {plan.price === "0" ?
          <>
              <span style={{ fontSize: 44, fontWeight: 600, letterSpacing: -.02 }}>
                {lang === "fr" ? "Gratuit" : "Free"}
              </span>
            </> :

          <>
              <span style={{ fontSize: 44, fontWeight: 600, letterSpacing: -.02 }}>€{displayPrice}</span>
...
function Contact({ t, lang }) {
  ...
  const [form, setForm] = React.useState({ name: "", email: "", topic: t.contact.topicOptions[0], message: "" });
            ...
            <WindowChrome title={t.contact.signin.title} icon={<Ico.user style={{ color: "var(--accent)" }} />}>
              <div style={{ padding: 24 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <a href="auth/Login.html" className="btn btn-accent" style={{ textDecoration: "none" }}>{t.contact.signin.cta}<Ico.arrow style={{ width: 14, height: 14 }} /></a>
                  <a href="auth/Signup.html" className="btn" style={{ textDecoration: "none" }}>{t.cta.signup}</a>
                </div>
              </div>
            </WindowChrome>
          </div>
...
/* ============================================================
   FOOTER — taskbar-inspired bottom
   ============================================================ */
function Footer({ t, lang, setLang, taskbar }) {
  return (
    <footer style={{ paddingTop: 64, borderTop: "1px solid var(--bd-divider)", background: "rgba(0,0,0,.2)" }}>
      <div className="container">
        <div className="footer-grid">
          <div>
            ...
            <div style={{ marginTop: 24, display: "flex", gap: 10 }}>
              <div style={{ fontSize: 11, color: "var(--tx-quaternary)", letterSpacing: ".06em", textTransform: "uppercase" }}>
                {t.footer.madeIn}
              </div>
            </div>
          </div>
          ...
          <div key={i}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--tx-primary)", letterSpacing: ".04em", textTransform: "uppercase", marginBottom: 14 }}>{col.title}</div>
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 10 }}>
                {col.items.map((it, k) =>
              <li key={k}>
                    <a href="#" style={{ fontSize: 14, color: "var(--tx-tertiary)", transition: "color .12s" }}
        ...
        {/* Optional taskbar at bottom — only when intensity >= "medium" */}
        {taskbar &&
        <div style={{
          margin: "48px 0 0",
          ...
          paddingBottom: 16
            ...
            <div className="taskbar">
              ...
              <button className="tb-btn active" aria-label="Consilium">
                <BrandMark size={22} />
              </button>
...
Object.assign(window, { Templates, UseCases, HowItWorks, Pricing, Reviews, Contact, Footer });